...
const iconsSorting = {
	up: (
		<svg
			...
			viewBox="0 0 40 40"
		>
			<path d="M 8 16 L 20 4 L 32 16 z" />
		</svg>
	),
	down: (
		<svg
			...
			viewBox="0 0 40 40"
		>
			<path d="M 8 24 H 32 H 32 L 20 36 z" />
		</svg>
	),
	middle: (
		<svg
			...
			viewBox="0 0 40 40"
		>
			<path d="M 8 16 L 20 4 L 32 16 z M 8 24 H 32 H 32 L 20 36 z" />
		</svg>
	),
};
...
function TableComponent({ data, onChange, visibility }) {
	...
	//didmount
	useEffect(() => {
		if (Object.keys(sortTableState).length === 0) {
			const sortTable = {};
			titles.map((v) => {
				sortTable[v] = 0;
			});
	...
	useEffect(() => {
		...
		if (index) {
			...
			if (type === "string")
				...
				dataTmp.sort((a, b) => {
					if (sortTableState[index] === 1) return a[index] - b[index];
					if (sortTableState[index] === -1)
	...
	const makeArrow = (e) => {
		if (sortTableState[e] === 0) return iconsSorting.middle;
		if (sortTableState[e] === 1) return iconsSorting.up;
		if (sortTableState[e] === -1) return iconsSorting.down;
	};
		...
		dataTmp.length > 0 && (
			<table className="Table">
				<thead className="Table-head">
					<tr>
						{titles.map((e, i) => (
							<th key={i} onClick={() => sortTable(e)}>
								{e} <div className="Arrow">{makeArrow(e)}</div>
							</th>
						))}
					</tr>
				</thead>
						...
						<tr key={i} style={{ color: p.color }}>
							{titles.map((v, k) =>
								v === visibility ? (
									...
									<td key={k} className="Table-block">
										{p[v]}
									</td>
								)
							)}
